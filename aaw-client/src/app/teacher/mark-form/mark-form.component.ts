...
export class MarkFormComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.activeRoute.params.subscribe(p => {
      if (p && p.id && p.id != 0) {
        this._baseService.getRecord(environment.segmentUrls.teacher.marks, p.id)
          .subscribe(v => {
            this.record = v.data;
    ...
    this.filteredCourses = courseValues.pipe(
      ...
      flatMap(x => {
        return this._baseService.getRecords(environment.segmentUrls.teacher.courses, 0, null, null, x)
    ...
    this.filteredStudents = studentValues.pipe(
      ...
      distinctUntilChanged(),
      flatMap(x => {
        return this._baseService.getRecords(environment.segmentUrls.teacher.students, 0, null, null, x)
          .pipe(map(y => y.data.records));
      }));
  ...
  onSubmit() {
    ...
    if (this.record.id == undefined) {
      obs = this._baseService.insertRecord(environment.segmentUrls.teacher.marks, this.record);

    } else {
      obs = this._baseService.updateRecord(environment.segmentUrls.teacher.marks, this.record);
    }
